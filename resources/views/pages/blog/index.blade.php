@extends('layouts.site')




@push('metaTitleDescrKeys')
	<title>{{ $meta->blog_title ?? '' }}</title>
	<meta name="description" content="{{ $meta->blog_description ?? '' }}">
	<meta name="keywords" content="{{ $meta->blog_keywords ?? '' }}">
	<link rel="canonical" href="{{ $meta->blog_canonical ?? '' }}" />
	<meta name="author" content="{{ $meta->blog_avtor ?? '' }}">
@endpush






@section('content')

<section class="text-gray-600 body-font">
  <div class="container px-5 py-5 mx-auto">

		<nav class="text-black font-bold my-8 mx-4 sm:mx-0" aria-label="Breadcrumb">
			<ol class="list-none p-0 inline-flex">
				<li class="flex items-center">
					<a class="font-medium hover:text-blue-700 transition" href="{{ route('home.page') }}">
						Главная
						</a>
					<svg class="fill-current w-3 h-3 mx-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"/></svg>
				</li>
				<li>
					<a class="text-gray-400 font-light" aria-current="page">
						{{ $profile->blog_title ?? 'Блог / Новости' }}
						</a>
				</li>
			</ol>
		</nav>


    



		<div class="flex flex-col">
      <div class="h-1 bg-gray-200 rounded overflow-hidden">
        <div class="w-24 h-full bg-indigo-500"></div>
      </div>
      <div class="flex flex-wrap sm:flex-row flex-col py-6 mb-2">
        <div class="sm:w-2/5 text-gray-900 font-medium title-font text-2xl sm:text-3xl mb-2 sm:mb-0">
	        {{ $profile->blog_title ?? 'Блог / Новости' }}
	        </div>
	       {!! $profile->blog_subtitle ?? 'нет данных' !!}
<!--
        <p class="sm:w-3/5 leading-relaxed text-base sm:pl-10 pl-0">
					{{ $profile->blog_subtitle ?? 'нет данных'}}
				</p>
-->
      </div>
    </div>





    <div class="flex flex-wrap -m-4">


			@foreach ($blogs as $blog)
					
      <div class="xl:w-1/4 md:w-1/2 p-4">
        <div class="bg-gray-100 p-6 rounded-lg hover:shadow-xl transition">
         <a href="{{ route('blog.detail', $blog->slug) }}">
					<img class="h-40 rounded w-full object-cover object-center mb-6" src="{{ asset($blog->pic ?? 'uploads/not_image.jpg') }}" alt="content">
          <h2 class="text-lg text-blue-900 font-medium title-font mb-4 sm:h-20 overflow-hidden">{{ $blog->title }}</h2>
          <div class="block text-base h-24 ">{!! Str::limit($blog->short, 100) !!}</div>
				 </a>
				 <a href="{{ route('blog.detail', $blog->slug) }}" class="bg-blue-900 hover:bg-blue-700 transition py-2 px-7 text-white text-xs uppercase rounded-md mt-5 inline-block">Перейти</a>
        </div>
      </div>
			

			@endforeach



    </div>
  </div>
</section>



@if(isset($seo->blogpage_title))
<div class="container mx-auto border-t border-gray-200 mt-10 px-5 sm:px-0">
	<h1 class="mt-10 mb-10 text-3xl font-semibold text-gray-500">
		{{ $seo->blogpage_title ?? '' }}
	</h1>
	<p class="text-lg">
		{!! $seo->blogpage_txt ?? '' !!}
	</p>
</div>
@endif

@endsection