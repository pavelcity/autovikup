...
@section('title', 'Редактировать | Создать')
...
<div class="card radius-15">
	<div class="card-body">
		...
		<form action="{{ route('dashboard.otzivy.update', $otziv->id) }}" method="post" autocomplete="off" enctype="multipart/form-data">
			@csrf



			<input type="hidden" name="old_pic" value="{{ $otziv->pic }}">

			<div class="mt-4">
				<img id="showPic" style="width: 400px; margin-bottom:30px;" src="{{ asset($otziv->pic ?? 'uploads/not_image.jpg') }}" alt="">
			</div>
			...
			<div class="mb-3">
				...
				<input class="form-control @error('title') error_line @enderror" id="title" name="title" type="text" placeholder="" value="{{ $otziv->title }}">
				@error('title')
			...
			<div class="mb-3">
				...
				<input class="form-control @error('dolznost') error_line @enderror" id="dolznost" name="dolznost" type="dolznost" placeholder="" value="{{ $otziv->dolznost }}">
				@error('dolznost')
					<p class="mistake">{{ $message }}</p>
				@enderror
			</div>
			...
			<div class="mb-5">
				...
				@error('text')
					<p class="mistake">{{ $message }}</p>
				@enderror
				<textarea id="text" name="text" class="form-control" rows="10" cols="3">{{ $otziv->text }}</textarea>
			</div>
